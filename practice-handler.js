// Practice Handler
// This file manages the display and checking of practice questions
...
async function loadQuestions() {
    try {
        const response = await fetch('questions-bank.json');
        if (!response.ok) {
...
// Initialize practice session RANDOMISER - PUT BACK IN POST PRACTICE
async function initializePractice(sourceType, distribution) {
    if (!questionBank) {
...
// Non randomised version for testing
...
// Get questions for a specific source type (in order, not randomized)
function getRandomQuestions(sourceType, distribution) {
    ...
    for (const [category, count] of Object.entries(distribution)) {
        ...
        // Removed randomization - questions now appear in original order
        const selected = categoryQuestions.slice(0, count);
        selectedQuestions.push(...selected);
    }
    
    // Return questions in order (no final shuffle)
